...
if(!function_exists('navItem')){
	function navItem($title,$href = null,$active = null,$nofollow = null){
		...
		if(!is_numeric($title) && GetMessage($title)){
			$title = GetMessage($title);
		}
		ob_start();
	?><li class="mx-auto <?=implode(' ',$arLiClasses)?>"><a href="<?=$href?>" class="page-link"<?if($nofollow){?>  rel="nofollow"<?}?>><?=$title?></a></li><?
		return ob_get_clean();
	}
}
...
if($arResult["bDescPageNumbering"] === true){
	...
	echo navItem('<i class="fa fa-chevron-left" aria-hidden="true"></i>',$prevLink);
	...
	echo navItem('<i class="fa fa-chevron-right" aria-hidden="true"></i>',$nextLink);
